...
| # | 查询 | 预期意图 | 测试目标 |
|---|------|---------|---------|
| 15 | 最近7天的GMV | 7天范围 | 时间范围识别 |
| 16 | 本月营收 | 本月范围 | 相对时间 |
| 17 | 2023年GMV | 2023全年 | 绝对时间 |
| 18 | GMV总和 | SUM聚合 | 聚合类型识别 |
| 19 | 平均客单价 | AVG聚合 | 平均值识别 |
| 20 | 用户数量 | COUNT聚合 | 计数识别 |
| 21 | 按用户的GMV | 维度=用户 | 维度识别 |
| 22 | GMV同比 | yoy比较 | 比较类型识别 |
| 23 | 电商GMV | 域过滤=电商 | 业务域过滤 |
| 24 | 实时DAU | 实时数据 | 新鲜度过滤 |
...
| # | 查询 | 预期行为 | 测试目标 |
|---|------|---------|---------|
| 25 | 嗯那个呃这个GMV总和 | 忽略噪音词 | 噪音词过滤 |
| 26 | 最近7天本月GMV | 识别第一个时间 | 冲突时间处理 |
| 27 | GMV总额总计合计 | 识别SUM聚合 | 冗余词处理 |
| 28 | GMV最高最低 | 识别第一个聚合 | 相反聚合处理 |
| 29 | 3月份GMV | 正确处理数字 | 数字干扰 |
| 30 | 查询一下最近的GMV | 提取"GMV" | 口语化处理 |
| 31 | GMV增长率的变化率 | 识别RATE | 嵌套关键词 |
| 32 | 按用户按地区的GMV | 识别多个维度 | 多维度处理 |
| 33 | 最经7天的GMV | 容错处理 | 错别字处理 |
| 34 | 实时电商用户GMV | 多条件过滤 | 复合条件 |
...
- 🔍 **核心查询词**：去除时间等干扰信息后的查询
- 📅 **时间范围**：识别的起止日期
- ⏰ **时间粒度**：day/week/month/year等
- 🧮 **聚合类型**：sum/avg/count/max/min等
- 📊 **分析维度**：按用户/地区/品类等
- 📈 **比较类型**：同比/环比/日环比等
- 🔧 **过滤条件**：实时/历史/业务域等
